...
public class AIButton extends SoundLevelButton implements AIListener {
    ...
    private void init(final Context context, final AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray viewAttrs = context.obtainStyledAttributes(attrs, ai.api.R.styleable.SoundLevelButton);
            try {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (animationStage > 0 || animationSecondPhase) {
            ...
            paint.setColor(getResources().getColor(ai.api.R.color.icon_orange_color));
    ...
    public enum MicState {
        ...
        public static MicState fromAttrs(final TypedArray viewAttrs) {
            if (viewAttrs.getBoolean(ai.api.R.styleable.SoundLevelButton_state_listening, false))
                return listening;
            if (viewAttrs.getBoolean(ai.api.R.styleable.SoundLevelButton_state_waiting, false))
                return busy;
            if (viewAttrs.getBoolean(ai.api.R.styleable.SoundLevelButton_state_speaking, false))
                return speaking;
            if (viewAttrs.getBoolean(ai.api.R.styleable.SoundLevelButton_state_initializing_tts, false))
