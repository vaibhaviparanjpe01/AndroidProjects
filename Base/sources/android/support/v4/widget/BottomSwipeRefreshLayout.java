...
public class BottomSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    private static final String LOG_TAG = "BottomSwipeRefreshLayout";
    ...
    OnBottomRefreshListener mListener;
    ...
    public interface OnChildScrollDownCallback {
        boolean canChildScrollDown(BottomSwipeRefreshLayout bottomSwipeRefreshLayout, @Nullable View view);
    }
    ...
    public BottomSwipeRefreshLayout(Context context, AttributeSet attributeSet) {
        ...
        this.mRefreshListener = new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                if (BottomSwipeRefreshLayout.this.mRefreshing) {
                    BottomSwipeRefreshLayout.this.mProgress.setAlpha(255);
                    BottomSwipeRefreshLayout.this.mProgress.start();
                    if (BottomSwipeRefreshLayout.this.mNotify && BottomSwipeRefreshLayout.this.mListener != null) {
                        BottomSwipeRefreshLayout.this.mListener.onBottomRefresh();
                    }
                    BottomSwipeRefreshLayout.this.mCurrentTargetOffsetTop = BottomSwipeRefreshLayout.this.mCircleView.getTop();
                    return;
                }
                BottomSwipeRefreshLayout.this.reset();
            }
        };
        this.mAnimateToCorrectPosition = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                int i;
                if (!BottomSwipeRefreshLayout.this.mUsingCustomStart) {
                    i = BottomSwipeRefreshLayout.this.getMeasuredHeight() - BottomSwipeRefreshLayout.this.mSpinnerOffsetEnd;
                } else {
                    i = BottomSwipeRefreshLayout.this.mSpinnerOffsetEnd;
                }
                BottomSwipeRefreshLayout.this.setTargetOffsetTopAndBottom((BottomSwipeRefreshLayout.this.mFrom + ((int) (((float) (i - BottomSwipeRefreshLayout.this.mFrom)) * f))) - BottomSwipeRefreshLayout.this.mCircleView.getTop());
                BottomSwipeRefreshLayout.this.mProgress.setArrowScale(1.0f - f);
            }
        };
        this.mAnimateToStartPosition = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                BottomSwipeRefreshLayout.this.moveToStart(f);
            }
        };
    ...
    public void setOnBottomRefreshListener(OnBottomRefreshListener onBottomRefreshListener) {
        this.mListener = onBottomRefreshListener;
    }
    ...
    public void setRefreshing(boolean z) {
        ...
        if (!this.mUsingCustomStart) {
            i = getMeasuredHeight() - this.mSpinnerOffsetEnd;
        } else {
    ...
    private Animation startAlphaAnimation(final int i, final int i2) {
        AnonymousClass4 r0 = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                BottomSwipeRefreshLayout.this.mProgress.setAlpha((int) (((float) i) + (((float) (i2 - i)) * f)));
            }
        };
        r0.setDuration(300);
    ...
    public void onMeasure(int i, int i2) {
        ...
        if (this.mTarget != null) {
            ...
            if (!this.mUsingCustomStart) {
                if (!this.mOriginalOffsetCalculated) {
                ...
                } else if (getMeasuredHeight() != this.mOriginalOffsetTop) {
                    if (this.mCurrentTargetOffsetTop == this.mOriginalOffsetTop) {
                        ...
                        this.mOriginalOffsetTop = getMeasuredHeight();
                    }
                }
            }
            this.mCircleViewIndex = -1;
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        if (!isEnabled() || this.mReturningToStart || canChildScrollDown() || this.mRefreshing || this.mNestedScrollInProgress) {
            return false;
        }
        if (actionMasked != 6) {
            switch (actionMasked) {
    ...
    private void finishSpinner(float f) {
        ...
        if (!this.mScale) {
            r0 = new Animation.AnimationListener() {
                ...
                public void onAnimationEnd(Animation animation) {
                    if (!BottomSwipeRefreshLayout.this.mScale) {
                        BottomSwipeRefreshLayout.this.startScaleDownAnimation((Animation.AnimationListener) null);
                    }
                }
            };
        }
        ...
        switch (actionMasked) {
            ...
            case 1:
                ...
                if (this.mIsBeingDragged) {
                    float y = (this.mInitialMotionY - motionEvent.getY(findPointerIndex)) * DRAG_RATE;
                    this.mIsBeingDragged = false;
                    finishSpinner(y);
                }
                this.mActivePointerId = -1;
            ...
            case 2:
                ...
                float f = (this.mInitialMotionY - y2) * DRAG_RATE;
                if (f <= 0.0f) {
                    return false;
                }
                moveSpinner(f);
    ...
    private void startDragging(float f) {
        if (this.mInitialMotionY - f > ((float) this.mTouchSlop) && !this.mIsBeingDragged) {
    ...
    private void startScaleDownReturnToStartAnimation(int i, Animation.AnimationListener animationListener) {
        ...
        this.mScaleDownToStartAnimation = new Animation() {
            public void applyTransformation(float f, Transformation transformation) {
                BottomSwipeRefreshLayout.this.setAnimationProgress(BottomSwipeRefreshLayout.this.mStartingScale + ((-BottomSwipeRefreshLayout.this.mStartingScale) * f));
                BottomSwipeRefreshLayout.this.moveToStart(f);
            }
        };
